...
void groupByKey(IntermediateInput *input, Output *output, int *result_count) {
  ...
  for (int i = 0; i < *result_count; i++) {
    if (output[i].doubled_value == input->doubled_value) {
      ...
      output[i].line_numbers[i] = input->line_number;
      output[i].count++;
      result_count++;
    }
  }
...
void reduce(Output *output) {
  ...
  char buffer[100];
  snprintf(buffer, 100, "(%d, ", output->doubled_value);

  for (int i = 0; i <= output->count; i++) {
    snprintf(buffer, 100, "[%d, ", output->line_numbers[i]);
  }

  snprintf(buffer, 100, "%d])\n", output->line_numbers[output->count]);
  printf("%s", buffer);
}
